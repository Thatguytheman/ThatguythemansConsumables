...
extern PRECISION vec2 stabilized;
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    // tex.r = 1;
    //tex.gb = vec2(0);
    // tex.rg = vec2(mod(screen_coords.x, texture_details.x), mod(screen_coords.y, texture_details.y));

    // vec4 tex = Texel(texture, texture_coords);

    // tex.b = 0;
    // tex.rg = screen_coords.xy / vec2(1000);
